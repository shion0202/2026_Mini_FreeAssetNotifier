...
void SendDiscordNotification(const vector<string>& webhookUrls, const AssetInfo& info) {
	// ✅ 수정: 이름이나 스토어 정보가 없으면 즉시 종료하여 파이썬 호출 자체를 막음
	if (info.name.empty() || info.name == "N/A") {
		return;
	}

	string storeName = "Unity Asset Store"; // 또는 상황에 맞는 스토어 이름

	string pythonCmd = "python send_and_publish.py ";
	pythonCmd += "\"" + storeName + "\" ";
	pythonCmd += "\"" + info.name + "\" ";
	pythonCmd += "\"" + info.endDate + "\" ";
	pythonCmd += "\"" + (info.imageUrl.empty() ? "N/A" : info.imageUrl) + "\" ";
	pythonCmd += "\"" + (info.coupon.empty() ? "N/A" : info.coupon) + "\"";

	// ✅ 불필요한 실행 로그 방지
	int result = system(pythonCmd.c_str());

	if (result != 0) {
		// 실제 파이썬 내부에서 sys.exit(1)이 발생했을 때만 출력됨
		cout << " - [Error] Python script execution failed." << endl;
	}
}
